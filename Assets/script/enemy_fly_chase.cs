...
public class enemy_fly_chase : MonoBehaviour
{
    public Rigidbody2D enemy_rigid_2d;
    public Vector2 move_par;
    public float scale;
    ...
    {
        enemy_rigid_2d.velocity = move_par;//new Vector2(move_par, enemy_rigid_2d.velocity.y);

        /*
        Vector2 frontVec = new Vector2(enemy_rigid_2d.position.x + move_par.normalized.x * scale, enemy_rigid_2d.position.y);
        ...
        if (raycast.collider != null && go_left)
        {
           ...
           // move_par = Vector2.left;
        }
        else if (raycast.collider != null && go_right)
        {
            CancelInvoke();
            //move_par = Vector2.right;
        }
    ...
    {
        move_par.x = Random.Range(-1, 1);
        move_par.y = Random.Range(-1, 1);
